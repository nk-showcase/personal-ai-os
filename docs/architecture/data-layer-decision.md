...
**Yes — the ADR's SQLite assumption is correct and is now affirmed, not merely assumed.** Scheme 3 (per-row AEAD, structured columns cleartext, `age`-wrapped KEK) is engine-agnostic and is the right design *because* it stays above SQLite rather than depending on engine TDE that Postgres doesn't have and SQLCipher can't do selectively (`adr:26-41`). **One correction the ADR must absorb (it already half-states it at `adr:82`, `:307`):** the load-bearing isolation control is the **separate unix users**, not the encryption — and that split is **not built yet** (all services `User=ai-os`). The ADR should be annotated that its barrier #4 ("0600 + right owner") is a **no-op until the `ai-os-bot` / `ai-os-worker` split exists** (`adr` Part B Phase 0, `:143`, owner step `:266`). This is the single most important hardening and it is an OS/systemd change, not a database change.
