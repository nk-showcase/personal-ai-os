...
| Root-owned integrations code isolation | shipped-as-code (deployment) | `scripts/provision_integrations_isolation.sh` creates + locks a root-owned code/venv tree the coding-worker user cannot write, and `systemd/aios-integrations-worker.service` runs from it — so a compromised coding worker cannot swap the code the keyed worker executes. See §4 |
...
- **Auto-restart with a compile gate.** Services restart on failure, but a restart only takes new code that byte-compiles, so a syntax-broken edit cannot wedge the system into a crash loop.
- **Trust-separated code, not just credentials.** The keyed integrations worker runs its code and Python interpreter from a **root-owned tree the coding-worker user cannot write** — `scripts/provision_integrations_isolation.sh` creates and locks that tree, and `systemd/aios-integrations-worker.service` runs from it. So a compromised coding worker cannot swap in code that the keyed worker would execute on its next restart (which would otherwise hand over the integration keys and the store key). This complements the three-separate-OS-accounts split (§3).
- **Live-deployment hardening (beyond this reference tree).** The production deployment additionally signs each approval verdict — the transport signs an `Allow` with an Ed25519 key the coding worker does not have, and the worker verifies it — so the gated worker cannot forge its own approval by writing the shared queue directly. That signing is deliberately **not** included in this reference tree, to keep the transport tier genuinely keyless here; it is documented as a deployment hardening.
...
- Follow `docs/vps/` and `systemd/` for the always-on service units. The units use `${AIOS_HOME}`-style placeholders (systemd does not expand these itself), so render + install them with `scripts/install_systemd_units.sh` (root), which substitutes the concrete paths and reloads systemd.
- Run `scripts/provision_integrations_isolation.sh` (root) to create the root-owned code tree the integrations worker runs from (isolated from the coding worker). See §4.
